...
from grape.module import GNNStack, MLPNet
...
class GrapeImputation():
    def __init__(self,
        ...
        print(model_types, norm_embs, post_hiddens)
        # build model
        self.model = GNNStack(n_features, 
                        1,
                        node_dim, 
                        edge_dim, 
                        1,
        ...
        self.impute_model.to(self.device)
    ...
    def prepare_data(self, data):
        ################ GRAPE use a edge_attr with shape of [E, 1]
    ...
    def fit(self, df_miss:pd.DataFrame, idx_train=None, idx_val=None, n_epochs = 1000, lr = 1e-3, weight_decay = 1e-4):
        ...
        for epoch in range(n_epochs):
            ...
            optimizer.zero_grad()
            x_embd = self.model(data)
            
            ######################################   construct inputs for impute_model   ######################################
            node_emb = x_embd[:data.n_units]
            attr_emb = x_embd[data.n_units:]
            ...
            imputed_attrs = self.impute_model(combined_emb)
            outputs = {'imputed_attrs': imputed_attrs}
    ...
    def transform(self, df_miss:pd.DataFrame):
        ...
        with torch.no_grad():
            ...
            imputd_attributes = self.impute_model(combined_emb).squeeze()     # [E, 1]
            imputd_attributes[data.observed_mask] = data.edge_attr.squeeze()[: len(data.edge_attr)//2]
        
        n_units, n_features = df_miss.shape
        imputed_array = imputd_attributes.reshape(n_units, n_features).cpu().numpy()

        return imputed_array
        # outputs = {'imputed_attrs': imputed_array}
        # return outputs
