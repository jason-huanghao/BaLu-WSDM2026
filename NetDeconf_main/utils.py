...
def sparse_mx_to_torch_sparse_tensor(sparse_mx,cuda=False):
    ...
    sparse_tensor = torch.sparse_coo_tensor(indices, values, shape, dtype=torch.float)
    if cuda:
...
def load_data_mat(dataset_mat_fn:str, original_X = False):
    # print(os.path.exists(dataset_mat_fn), ":", dataset_mat_fn)
    ...
    if "train_idx" in data:
        # big issue !!!!!!!!!!!!!! with the sp.savemat(), it will automatically transform [N] numpy.arrray into shape of [1, N]
        idx_train, idx_test, idx_val = data['train_idx'].squeeze(), data['test_idx'].squeeze(), data['val_idx'].squeeze()  
        # print("idx_train load() shape:", idx_train.shape)
    else:
        idx_train, idx_test, idx_val = None, None, None
    
    # variables = [X, A, T, Y1, Y0, idx_train, idx_test, idx_val]
    # names = ['X', 'A', 'T','Y1', 'Y0', 'idx_train', 'idx_test', 'idx_val']
    ...
    Y = data['Y']
    return X, A, T, Y, Y1, Y0, idx_train, idx_test, idx_val
