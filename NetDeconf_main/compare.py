...
def to_device(*obs):
    ...
    return tuple(ob.to(device) if isinstance(ob, torch.Tensor) else ob for ob in obs)
...
def prepare_data(dataset_path, imputed_version=True):
    ...
    if imputed_version: 
        ...
        # keep_non_null = ~torch.tensor(data.df_imputed.isnull().any(axis=1).to_numpy(), dtype=torch.bool)    # all non-null train, validation, and test
        # keep_non_null_and_test = keep_non_null | idx_test                                                   # all test
